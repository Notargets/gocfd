...
On the path to implementing direct flux reconstruction, I found what appeared to be 2nd order aliasing without a clear origin. After consulting Hesthaven(2007) section 5.3, I see that the issue is the interpolation of the flux and subsequently taking the derivative of that interpolated flux. As stated: "the derivative of the interpolation is not the same as the interpolation of the derivative", or put another way, by simply computing the flux from the nodal points of the solution polynomial, we are not treating the flux formally as a polynomial - instead we should perform a formal polynomial fit of the flux prior to using that polynomial to compute derivatives of the flux. The result shown in the text is that we introduce an aliasing error into the solution, and their answer is to filter it away (not to do the much more compute intensive fit) - which I've done already using a simple 2nd order dissipative filter, which works well. I also experimented with a combined 2nd / 4th order filter, similar to what is commonly used in finite volume schemes and for this problem the 2nd order dissipation was enough.

With these results, I'm comfortable moving on to implementing DFR for Euler in 1D, where I'll probably investigate more advanced flux computations (Roe, etc) and numerical dissipation forms.
