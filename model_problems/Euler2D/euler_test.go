...
func TestEuler(t *testing.T) {
	if true {
		{ // Test interpolation of solution to edges for all supported orders
			Nmax := 7
			for N := 1; N <= Nmax; N++ {
				c := NewEuler(1, N, "../../DG2D/test_tris_5.neu", 1, FLUX_Average, FREESTREAM, 0, false, false)
				Kmax := c.dfr.K
		...
		{ // Test divergence of polynomial initial condition against analytic values
			...
			*/
			...
			for N := 1; N <= Nmax; N++ {
				...
				var c *Euler
				c = NewEuler(1, N, "../../DG2D/test_tris_1tri.neu", 1, FLUX_Average, FREESTREAM, 0, plotMesh, false)
				CheckFlux0(c, t)
				// Two widely separated triangles - no shared faces
				c = NewEuler(1, N, "../../DG2D/test_tris_two.neu", 1, FLUX_Average, FREESTREAM, 0, plotMesh, false)
				CheckFlux0(c, t)
				// Two widely separated triangles - no shared faces - one tri listed in reverse order
				c = NewEuler(1, N, "../../DG2D/test_tris_twoR.neu", 1, FLUX_Average, FREESTREAM, 0, plotMesh, false)
				CheckFlux0(c, t)
	...
	if true {
		{ // Test divergence of Isentropic Vortex initial condition against analytic values - density equation only
			N := 1
			plotMesh := false
			c := NewEuler(1, N, "../../DG2D/test_tris_6.neu", 1, FLUX_Average, IVORTEX, 0, plotMesh, false)
...
func TestEuler_Solve(t *testing.T) {
	...
	N := 2
	plotMesh := false
	c := NewEuler(0.1, N, "../../DG2D/vortexA04.neu", 1.00, FLUX_LaxFriedrichs, IVORTEX, 0, plotMesh, true)
...
func BenchmarkEuler_Solve(b *testing.B) {
	...
	for n := 1; n <= Nmax; n++ {
		c[n] = NewEuler(FinalTime, n, "../../DG2D/vortex-new.su2", 1.00, FLUX_LaxFriedrichs, IVORTEX, 0, plotMesh, false)
	}
	b.ResetTimer()
	// The benchmark loop
	for i := 0; i < b.N; i++ {
