...
func TestPlotProjection(t *testing.T) {
	...
	c.InterpolateSolutionToShockedEdges(c.ShockFinder, Q, Q_Face, Q_Face_P0)
	for _, k := range c.ShockFinder.ShockCells.Cells() {
		// fmt.Println("Shocked cell K = ", k)
		// fmt.Println("Q_Face[K] = ", Q_Face[n].Col(k))
...
func TestEdgeMarking(t *testing.T) {
	var (
		N = 4
	)
	if !testing.Verbose() {
	...
	// gp.MassMatrix.Print("MM")
	// gp.Minv.Print("Minv")
	NpGraph, _ := c.DFR.GraphInterp.Dims()
	var Q, Q_Face, QInterp [4]utils.Matrix
	for n := 0; n < 4; n++ {
		Q[n] = utils.NewMatrix(dfr.SolutionElement.Np, dfr.K)
		Q_Face[n] = utils.NewMatrix(dfr.FluxElement.NpEdge*3, dfr.K)
		QInterp[n] = utils.NewMatrix(NpGraph, dfr.K)
	}
	// SetTestFieldQ(dfr, RADIAL1TEST, Q)
	// SetTestFieldQ(dfr, NORMALSHOCKTESTM12, Q)
	// SetTestFieldQ(dfr, NORMALSHOCKTESTM18, Q)
	...
	n := 0
	// for n := 0; n < 4; n++ {
	dfr.GraphInterp.Mul(Q[n], QInterp[0])
	c.DFR.FluxEdgeProject0Interp.Mul(Q[0], Q_Face[0])
