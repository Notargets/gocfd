...
func BenchmarkEulerSolve(b *testing.B) {
	...
	for n := 1; n <= Nmax; n++ {
		c[n] = Euler2D.NewEuler(FinalTime, n, "../../../DG2D/vortex-new.su2", 1.00, Euler2D.FLUX_LaxFriedrichs, Euler2D.IVORTEX, 0, 0, 1.4, 0, false, 5000, plotMesh, false)
	}
	b.ResetTimer()
	// The benchmark loop
	for i := 0; i < b.N; i++ {
...
func BenchmarkEulerGetFlowFunction(b *testing.B) {
	var (
		q = [4]float64{1, 1, 1, 1}
		c = Euler2D.NewEuler(1, 1, "", 1, Euler2D.FLUX_LaxFriedrichs, Euler2D.FREESTREAM,
			1, 1, 1.4, 0, true, 1,
			false, false)
		GM1 = c.Gamma - 1
	)
	...
	b.Run("Optimized function call", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			p = c.GetFlowFunction(q, Euler2D.StaticPressure)
		}
	})
