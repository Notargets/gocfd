...
func TestLocalNeighborReciprocity(t *testing.T) {
...
func TestLocalNeighborPhysicalCoordinates(t *testing.T) {
	meshPath := getTestMeshPath()
	el, err := tetrahedra.NewElement3D(1, meshPath) // Order 1 for simpler testing
	require.NoError(t, err)
	require.NotNil(t, el.SplitElement3D, "Need split mesh")
...
func TestIndexRangeValidity(t *testing.T) {
	meshPath := getTestMeshPath()
	el, err := tetrahedra.NewElement3D(3, meshPath) // Higher order for more face points
	require.NoError(t, err)
	require.NotNil(t, el.SplitElement3D, "Need split mesh")
...
func getFacePointCoordinates(el *tetrahedra.Element3D, elem, face, point uint32) (x, y, z float64) {
