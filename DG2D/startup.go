...
func (el *Elements2D) Startup2D(nt NODE_TYPE) {
	var (
		err error
		N   = el.Np - 1
	)
	...
	el.V = Vandermonde2D(N, el.R)
	if el.Vinv, err = el.V.Inverse(); err != nil {
		fmt.Println(err)
		panic("error inverting V")
	}
	Vr := GradVandermonde2D(el.R, N)

	el.Dr = Vr.Mul(el.Vinv)

	el.LIFT = Lift2D(el.V, el.Np, el.NFaces, el.Nfp)

	el.NX = Normals2D(el.NFaces, el.Nfp, el.K)
	...
	var J utils.Matrix
	J, el.Rx = GeometricFactors2D(el.Dr, el.X)
...
func (el *Elements2D) FaceMap() (VmapM, VmapP utils.Index) {
	/*
