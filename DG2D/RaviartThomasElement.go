...
// Below is an implementation of a 2D orthogonal polynomial parameterized to
// vary over j such that the j-th polynomial is orthogonal to it's cohort
func Compute2DOrthogonalPolynomial(R, S utils.Vector, j, P int) []float64 {
	Np := (P + 1) * (P + 2) / 2

	// Compute (p, q) from index j
	p := int(math.Floor((math.Sqrt(8*float64(j)+1) - 1) / 2))
	q := j - (p*(p+1))/2 - 1

	// Define alpha, beta parameterized by j
	alpha := float64(j)/float64(Np) - 0.5
	beta := float64(j)/float64(Np) - 0.5

	// Compute Jacobi polynomials
	Pp := DG1D.JacobiP(R, alpha, beta, p)
	Pq := DG1D.JacobiP(S, 2*float64(p)+1, beta, q)

	// Compute polynomial basis function
	Bj := make([]float64, len(R.DataP))
	for i := range Bj {
		Bj[i] = Pp[i] * Pq[i] * math.Pow(1-S.DataP[i], float64(p))
	}
	return Bj
}

func Compute2DOrthogonalPolynomialGradient(R, S utils.Vector, j, P int) (ddr, dds []float64) {
	Np := (P + 1) * (P + 2) / 2
	...
	// Compute Jacobi polynomials
	Pp := DG1D.JacobiP(R, alpha, beta, p)
	dPp := DG1D.GradJacobiP(R, alpha, beta, p) // ∂/∂r P_p

	Pq := DG1D.JacobiP(S, 2*float64(p)+1, beta, q)
	dPq := DG1D.GradJacobiP(S, 2*float64(p)+1, beta, q) // ∂/∂s P_q

	// Allocate memory for derivatives
	ddr = make([]float64, len(R.DataP))
	dds = make([]float64, len(R.DataP))

	// Compute derivatives at each evaluation point
	for i := range ddr {
		// Compute ∂/∂r
		ddr[i] = dPp[i] * Pq[i] * math.Pow(1-S.DataP[i], float64(p))

		// Compute ∂/∂s
		dds[i] = Pp[i]*dPq[i]*math.Pow(1-S.DataP[i], float64(p)) -
			float64(p)*Pp[i]*Pq[i]*math.Pow(1-S.DataP[i], float64(p-1))
	}
