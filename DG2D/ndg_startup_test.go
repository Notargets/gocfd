...
func TestNDG2D(t *testing.T) {
	{
		N := 1
		x, y := Nodes2D(N)
		assert.InDeltaSlicef(t, []float64{-1, 1, 0}, x.DataP, 0.0001, "")
		assert.InDeltaSlicef(t, []float64{-0.5774, -0.5774, 1.1547}, y.DataP, 0.0001, "")

		N = 2
		x, y = Nodes2D(N)
		assert.InDeltaSlicef(t, []float64{-1, 0, 1, -.5, .5, 0}, x.DataP, 0.0001, "")
		assert.InDeltaSlicef(t, []float64{-0.5774, -0.5774, -0.5774, 0.2887, 0.2887, 1.1547}, y.DataP, 0.0001, "")
		R, S := XYtoRS(Nodes2D(N))
		assert.InDeltaSlicef(t, []float64{-1, 0, 1, -1, 0, -1}, R.DataP, 0.0001, "")
		assert.InDeltaSlicef(t, []float64{-1, -1, -1, 0, 0, 1}, S.DataP, 0.0001, "")
		a, b := RStoAB(R, S)
		assert.InDeltaSlicef(t, []float64{-1, 0, 1, -1, 1, -1}, a.DataP, 0.0001, "")
		assert.InDeltaSlicef(t, []float64{-1, -1, -1, 0, 0, 1}, b.DataP, 0.0001, "")
		...
		V2Dr, V2Ds := JB2D.GradVandermonde2D(N, R, S)
		assert.InDeltaSlicef(t, []float64{
	...
	{ // Warpfactor
		N := 3
		Np := (N + 1) * (N + 2) / 2
		warpf := Warpfactor(N, utils.NewVector(Np, []float64{-1.0000,
			-0.3333, 0.3333, 1.0000, -0.6667, -0.0000, 0.6667, -0.3333,
			0.3333, 0}))
		assert.InDeltaSlicef(t, []float64{0, -0.1281, 0.1281, 0, -0.2562, 0.0000, 0.2562, -0.1281, 0.1281, 0.0000}, warpf, 0.0001, "")
	}

	{ // Nodes2D - distribution
		N := 3
		x, y := Nodes2D(N)
		assert.InDeltaSlicef(t, []float64{-1.0000, -0.4472, 0.4472, 1.0000,
			-0.7236, -0.0000, 0.7236, -0.2764, 0.2764, 0}, x.DataP, 0.0001, "")
		assert.InDeltaSlicef(t, []float64{-0.5774, -0.5774, -0.5774, -0.5774,
			-0.0986, -0.0000, -0.0986, 0.6760, 0.6760, 1.1547}, y.DataP,
			0.0001, "")
		N = 4
		x, y = Nodes2D(N)
		assert.InDeltaSlicef(t, []float64{-1.0000, -0.6547, -0.0000, 0.6547,
			1.0000, -0.8273, -0.3274, 0.3274, 0.8273, -0.5000, 0.0000,
			0.5000, -0.1727, 0.1727, 0}, x.DataP, 0.0001, "")
		assert.InDeltaSlicef(t, []float64{-0.5774, -0.5774, -0.5774, -0.5774,
			-0.5774, -0.2783, -0.1890, -0.1890, -0.2783, 0.2887, 0.3780,
			0.2887, 0.8556, 0.8556, 1.1547}, y.DataP, 0.0001, "")
	}
	...
	{ // Read file to test specific metrics
		...
		assert.InDeltaSlicef(t, []float64{
			...
			2.7337, -10.4712, 15.5416, 19.2110,
			2.7337, -10.4712, 15.5416, 19.2110,
		}, ndg.Rx.Subset(subset.Range(":", "0:4"), 6, 4).DataP, 0.0001, "")
		assert.InDeltaSlicef(t, []float64{
			...
			-15.9949, 10.7477, 5.2533, -12.0803,
			-15.9949, 10.7477, 5.2533, -12.0803,
		}, ndg.Ry.Subset(subset.Range(":", "0:4"), 6, 4).DataP, 0.0001, "")
		assert.InDeltaSlicef(t, []float64{
			...
			14.1324, -14.8689, -12.2718, -1.3128,
			14.1324, -14.8689, -12.2718, -1.3128,
		}, ndg.Sx.Subset(subset.Range(":", "0:4"), 6, 4).DataP, 0.0001, "")
		assert.InDeltaSlicef(t, []float64{
			...
			4.4896, -10.7477, 10.6892, 16.5204,
			4.4896, -10.7477, 10.6892, 16.5204,
		}, ndg.Sy.Subset(subset.Range(":", "0:4"), 6, 4).DataP, 0.0001, "")
		...
		assert.InDeltaSlicef(t, []float64{
			...
			-0.1685, 0.6978, -0.9473, -0.8465,
			-0.1685, 0.6978, -0.9473, -0.8465,
		}, ndg.NX.Subset(subsetFacePts.Range(":", "0:4"), 9, 4).DataP, 0.0001, "")
		assert.InDeltaSlicef(t, []float64{
			...
			0.9857, -0.7163, -0.3202, 0.5323,
			0.9857, -0.7163, -0.3202, 0.5323,
		}, ndg.NY.Subset(subsetFacePts.Range(":", "0:4"), 9, 4).DataP, 0.0001, "")
	}
}
