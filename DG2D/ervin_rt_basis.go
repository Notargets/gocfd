...
func NewErvinRTBasis(P int, SingleEdgePts []float64) (e *ErvinRTBasis) {
	...
	case 2:
		e.g1_q, e.g2_q, e.g3_q = conv(SingleEdgePts[0]), conv(SingleEdgePts[1]),
			conv(SingleEdgePts[2])
...
func (e *ErvinRTBasis) ComposePhiRT2() (phi []BasisPolynomialTerm) {
	...
	var (
		...
		conv     = func(r float64) float64 { return (r + 1.) / 2. }
		dconvdrs = 1. / 2.
		div1     = 1. / ((e.g1_q - e.g2_q) * (e.g1_q - e.g3_q))
		div2     = 1. / ((e.g2_q - e.g1_q) * (e.g2_q - e.g3_q))
		div3     = 1. / ((e.g3_q - e.g1_q) * (e.g3_q - e.g2_q))

		q1Func = func(t float64) float64 {
			return (t - e.g2_q) * (t - e.g3_q) * div1
		}
		q2Func = func(t float64) float64 {
			return (t - e.g1_q) * (t - e.g3_q) * div2
		}
		q3Func = func(t float64) float64 {
			return (t - e.g1_q) * (t - e.g2_q) * div3
		}
		...
		q1xiEdge1 = BasisPolynomialMultiplier{
			Eval: func(r, s float64) float64 { return q1Func(conv(r)) },
			Gradient: func(r, s float64) (grad [2]float64) {
				grad = [2]float64{q1Deriv(conv(r)), 0}
				return
			},
		}
		q2xiEdge1 = BasisPolynomialMultiplier{
			Eval: func(r, s float64) float64 { return q2Func(conv(r)) },
			Gradient: func(r, s float64) (grad [2]float64) {
				grad = [2]float64{q2Deriv(conv(r)), 0}
				return
			},
		}
		q3xiEdge1 = BasisPolynomialMultiplier{
			Eval: func(r, s float64) float64 { return q3Func(conv(r)) },
			Gradient: func(r, s float64) (grad [2]float64) {
				grad = [2]float64{q3Deriv(conv(r)), 0}
				return
			},
		}

		q1etaEdge2 = BasisPolynomialMultiplier{
			Eval: func(r, s float64) float64 { return q1Func(conv(s)) },
			Gradient: func(r, s float64) (grad [2]float64) {
				grad = [2]float64{0, q1Deriv(conv(s))}
				return
			},
		}
		q2etaEdge2 = BasisPolynomialMultiplier{
			Eval: func(r, s float64) float64 { return q2Func(conv(s)) },
			Gradient: func(r, s float64) (grad [2]float64) {
				grad = [2]float64{0, q2Deriv(conv(s))}
				return
			},
		}
		q3etaEdge2 = BasisPolynomialMultiplier{
			Eval: func(r, s float64) float64 { return q3Func(conv(s)) },
			Gradient: func(r, s float64) (grad [2]float64) {
				grad = [2]float64{0, q3Deriv(conv(s))}
				return
			},
		}
